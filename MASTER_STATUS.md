# SecAI+ Course Project — Master Status

**Last Updated:** March 5, 2026
**Owner:** Kevin P. Heaney
**GitHub (public):** https://github.com/KPH3802/SecAI-Course-Project
**GitHub (private production):** https://github.com/KPH3802/SecAI_Course_Production

---

## PHASE TIMELINE

| Phase | Deliverable | Target Date | Status |
|-------|-------------|-------------|--------|
| 1 — Free Tier | Anki deck (50 cards) | Mar 2, 2026 | ✅ Done |
| 1 — Free Tier | Landing page + email capture | Mar 4, 2026 | ✅ Done |
| 1 — Free Tier | LinkedIn post | Mar 2, 2026 | ✅ Done |
| 1 — Free Tier | YouTube script | Mar 7, 2026 | ✅ Done |
| 1 — Free Tier | YouTube video recorded & uploaded | Mar 14, 2026 | ⬜ Not Started |
| 2 — Paid Tier | Practice exam (150 questions) | Mar 21, 2026 | ⬜ Not Started |
| 2 — Paid Tier | Gumroad listing — practice exam | Mar 22, 2026 | ⬜ Not Started |
| 2 — Paid Tier | Study guide (all 4 domains) | Apr 4, 2026 | ⬜ Not Started |
| 2 — Paid Tier | Gumroad listing — study guide | Apr 5, 2026 | ⬜ Not Started |
| 2 — Paid Tier | Bundle listing (exam + guide) | Apr 5, 2026 | ⬜ Not Started |
| 3 — Video Course | Module outlines (all domains) | Apr 18, 2026 | ⬜ Not Started |
| 3 — Video Course | Scripts — Domain 1 + 2 | Apr 25, 2026 | ⬜ Not Started |
| 3 — Video Course | Scripts — Domain 3 + 4 | May 2, 2026 | ⬜ Not Started |
| 3 — Video Course | Recording complete | May 16, 2026 | ⬜ Not Started |
| 3 — Video Course | Udemy/Coursera launch | May 23, 2026 | ⬜ Not Started |

---

## BUSINESS INFRASTRUCTURE

| Item | Status | Notes |
|------|--------|-------|
| Domain: secaiplus.com | ✅ Live | Registered at Namecheap, auto-renews Mar 2027 |
| GitHub Pages hosting | ✅ Live | Deployed from main branch root |
| HTTPS enforcement | ✅ Active | GitHub Pages SSL enforced |
| ConvertKit account | ✅ Active | Free plan, account: SecAI+ Study |
| Email: kevin@secaiplus.com | ✅ Active | Namecheap Private Email Starter, $11.88/yr |
| DNS — A Records (GitHub Pages) | ✅ Configured | 4 records pointing to 185.199.108-111.153 |
| DNS — CNAME (GitHub Pages) | ✅ Configured | www → kph3802.github.io |
| DNS — MX Records (Private Email) | ✅ Configured | mx1/mx2.privateemail.com, priority 10 |
| DNS — SPF TXT Record | ✅ Configured | v=spf1 include:spf.privateemail.com ~all |
| DNS — DKIM Record | ✅ Auto-configured | Added by Namecheap Private Email |
| ConvertKit sender address | ✅ Verified | kevin@secaiplus.com set as default sender |
| Gumroad account | ⬜ Not Started | Needed before paid tier launch — set up before Mar 21 |

---

## CONTENT — FREE TIER

### Anki Deck
| Item | Status | Notes |
|------|--------|-------|
| 50-card scenario-based deck | ✅ Complete | All 4 domains, weighted to exam % |
| Professional CSS styling | ✅ Complete | Color-coded domain badges, branded footer |
| Why wrong explanations | ✅ Complete | On every card |
| Exam tips | ✅ Complete | On every card |
| Published to GitHub Release | ✅ Live | v1.0.0 — SecAI_Course_Free_Deck.apkg |
| Download URL | ✅ Live | https://github.com/KPH3802/SecAI-Course-Project/releases/download/v1.0.0/SecAI_Course_Free_Deck.apkg |

### Landing Page
| Item | Status | Notes |
|------|--------|-------|
| HTML design & build | ✅ Complete | Dark theme, mint green accents |
| Hero form (ConvertKit) | ✅ Live | Form ID 9165815, data-uid 74f3166c72 |
| Bottom CTA form (ConvertKit) | ✅ Live | Fixed duplicate script bug Mar 5 |
| Full flow test | ✅ Passed | Email submits → confirmation → inbox delivery |
| Sender address | ✅ Professional | Sends from kevin@secaiplus.com |
| Inbox delivery (not junk) | ✅ Confirmed | Gmail inbox delivery confirmed |
| HTTPS | ✅ Enforced | |

### LinkedIn Post
| Item | Status | Notes |
|------|--------|-------|
| "I passed SecAI+" post | ✅ Published | Posted Mar 2, 2026 |

### YouTube Video #1
| Item | Status | Notes |
|------|--------|-------|
| Script | ✅ Complete | Saved to SecAI_Course_Production repo (private) |
| Recording | ⬜ Not Started | Target: Mar 14, 2026 |
| Upload & optimize (title, desc, thumbnail) | ⬜ Not Started | Target: Mar 14, 2026 |

### Brain Dump Recording
| Item | Status | Notes |
|------|--------|-------|
| Voice memo recorded | ✅ Complete | Post-exam brain dump |

---

## CONTENT — PAID TIER

### Practice Exam (150 Questions)
| Item | Status | Notes |
|------|--------|-------|
| Question bank (150 Qs, all 4 domains) | ⬜ Not Started | Build from 50 Anki Qs + 100 new |
| Full answer explanations + why-wrong | ⬜ Not Started | |
| PDF formatting & layout | ⬜ Not Started | |
| Gumroad listing | ⬜ Not Started | Target price: $19–29 |
| **Target date** | **Mar 21, 2026** | |

**Domain weighting for 150 questions (match exam %):**
- Domain 1 — AI Security Fundamentals: ~37 questions (25%)
- Domain 2 — Data and Model Security: ~37 questions (25%)
- Domain 3 — AI Security Operations: ~41 questions (27%)
- Domain 4 — Governance, Risk, Compliance: ~35 questions (23%)

### Study Guide PDF
| Item | Status | Notes |
|------|--------|-------|
| Domain 1 — AI Security Fundamentals | ⬜ Not Started | |
| Domain 2 — Data and Model Security | ⬜ Not Started | Weakest domain — allocate more time |
| Domain 3 — AI Security Operations | ⬜ Not Started | |
| Domain 4 — Governance, Risk, Compliance | ⬜ Not Started | ISO numbers section critical |
| PDF formatting & layout | ⬜ Not Started | |
| Gumroad listing | ⬜ Not Started | Target price: $39–49 |
| Bundle listing (exam + guide) | ⬜ Not Started | Target price: $59 |
| **Target date** | **Apr 5, 2026** | |

### Video Course
| Item | Status | Notes |
|------|--------|-------|
| Module outlines (all 4 domains) | ⬜ Not Started | Target: Apr 18, 2026 |
| Domain scripts (all 4) | ⬜ Not Started | Target: May 2, 2026 |
| Recording | ⬜ Not Started | Target: May 16, 2026 |
| Platform decision (Udemy vs Coursera) | ⬜ Not Started | Decide by Apr 18 |
| Launch | ⬜ Not Started | Target: May 23, 2026 |

---

## MARKETING & EMAIL

### ConvertKit Setup
| Item | Status | Notes |
|------|--------|-------|
| Form: SecAI Free Deck Signup | ✅ Active | |
| Incentive email | ✅ Active | Subject: "Your free SecAI+ Anki deck is here" |
| Deck download link in email | ✅ Working | GitHub Release v1.0.0 link |
| Double opt-in | ✅ Enabled | |
| Confirmed subscribers | ⏳ Growing | Test subscriber confirmed Mar 5 |
| Launch email — practice exam | ⬜ Not Started | Draft when exam is ready |
| Launch email — study guide | ⬜ Not Started | Draft when guide is ready |

---

## MONTHLY COSTS

| Service | Cost | Billing |
|---------|------|---------|
| secaiplus.com domain | ~$13/yr | Annual (Mar 2027) |
| Namecheap Private Email Starter | $11.88/yr | Annual (Mar 2027) |
| ConvertKit | $0 (free tier) | Monthly |
| GitHub Pages | $0 | Free |
| Gumroad | $0 + 10% fee | Per transaction |
| **Total fixed** | **~$2.08/mo** | |

---

## KEY URLS

| Resource | URL |
|----------|-----|
| Landing page | https://secaiplus.com |
| GitHub repo (public) | https://github.com/KPH3802/SecAI-Course-Project |
| GitHub repo (private) | https://github.com/KPH3802/SecAI_Course_Production |
| Anki deck download | https://github.com/KPH3802/SecAI-Course-Project/releases/download/v1.0.0/SecAI_Course_Free_Deck.apkg |
| ConvertKit dashboard | https://app.kit.com |
| Namecheap DNS | https://ap.www.namecheap.com |
| Private Email webmail | https://privateemail.com |
